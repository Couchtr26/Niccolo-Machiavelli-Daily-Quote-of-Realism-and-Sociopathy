...
<head>
  <meta charset="UTF-8">
  <title>☠️ Machiavellian Wisdom Generator ☠️</title>
  <style>
    body {
...
<body>
  ...
  <h1>☠️ Niccolò Machiavelli’s Daily Quote of Realism and Sociopathy ☠️</h1>

  <div id="quote">Click to reveal wisdom.</div>
  <div id="author"></div>

  <button class="button" onclick="showQuote()">Reveal Machiavellian Wisdom</button>
  <button class="button" onclick="saveQuote()">💾 Save Favorite Quote</button>

  <script>
    const niccolòMachiavelliQuote = [
    ...
    const niccolòMachiavelliSatansClusterfuck2025 = [
  ...
  if (useLatin) {
    return {
      quote: latinQuotes[Math.floor(Math.random() * latinQuotes.length)],
  ...
  if (random < 0.25) {
    ...
    return {
      quote: niccolòMachiavelliSatansClusterfuck2025[Math.floor(Math.random() * niccolòMachiavelliSatansClusterfuck2025.length)],
      author: "Niccolò Machiavelli Satans Clusterfuck 2025"
    };
  } else {
    return {
      quote: niccolòMachiavelliQuote[Math.floor(Math.random() * niccolòMachiavelliQuote.length)],
